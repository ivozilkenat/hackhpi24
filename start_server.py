...
def start_server(path=os.path.join(os.getcwd(), "server")):
    ...
    if sys.platform == "win32":
        ...
        run_command(f'cd {path} && .\\venv\\Scripts\\activate && uvicorn main:app --port=3001 --host 0.0.0.0')
    else:
        # Linux and macOS use this command
        run_command(f'cd {path} && . venv/bin/activate && uvicorn main:app --port=3001 --host 0.0.0.0')
